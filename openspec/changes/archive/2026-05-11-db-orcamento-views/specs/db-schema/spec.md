...
The database initialization SHALL execute `schema.sql` which includes both `CREATE TABLE IF NOT EXISTS` statements for all 6 raw data tables AND `CREATE VIEW IF NOT EXISTS` statements for the 11 financial views (`v_overview`, `v_bank_summary`, `v_credit_summary`, `v_investment_summary`, `v_monthly_cashflow`, `v_spending_by_cat`, `v_net_worth`, `v_investment_maturity`, `v_credit_alerts`, `v_top_categories_30d`, `v_budget_5030_20`).
...
- **AND** `SELECT name FROM sqlite_master WHERE type='view'` returns all 11 views
