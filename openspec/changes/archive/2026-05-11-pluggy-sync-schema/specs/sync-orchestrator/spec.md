...
O sistema SHALL persistir cada conjunto de entidades (ex: todas as transactions) dentro de uma única transação SQLite usando `db.transaction()`, garantindo atomicidade — ou todas as linhas são inseridas ou nenhuma.
