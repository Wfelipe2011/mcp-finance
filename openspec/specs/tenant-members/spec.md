...
O sistema SHALL ter tabela `tenant_members` com campos: `id` (SERIAL PK), `tenant_id` (UUID NOT NULL REFERENCES tenants(id)), `name` (TEXT NOT NULL — ex: "wilson"), `display_name` (TEXT NOT NULL — ex: "Wilson"), com constraint UNIQUE `(tenant_id, name)`. A tabela `d_users` SHALL ser removida.
