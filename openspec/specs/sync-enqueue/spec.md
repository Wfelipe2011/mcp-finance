...
O sistema SHALL ter lógica de `nextJob(workerId)` que: (1) sorteia aleatoriamente um `tenant_id` distinto com jobs `pending`, (2) seleciona o job com maior `date` desse tenant, (3) usa `FOR UPDATE SKIP LOCKED` para exclusividade, (4) atualiza atomicamente para `status='running'`, `worker_id`, `started_at`, `attempts++`.
...
O sistema SHALL executar `releaseStuck()` — que libera jobs com `status='running'` e `started_at < NOW() - 10 minutes` de volta para `pending` — antes de cada `nextJob()`.
