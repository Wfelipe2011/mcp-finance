...
O cron SHALL, para cada tenant ativo, executar `SET LOCAL app.tenant_id = <uuid>` e então consultar a cobertura do mês: `COUNT(*)` de `f_transacoes` vs `COUNT(ai.transaction_id)` via LEFT JOIN com `ai_transaction_insights`. Somente se `enriched = total AND total > 0` o digest é gerado.

#### Scenario: Query de cobertura retorna 100%
- **WHEN** `COUNT(*) = COUNT(ai.transaction_id)` e `total > 0` para o tenant/mês
- **THEN** o cron gera e persiste o digest para esse tenant

#### Scenario: Query de cobertura retorna parcial
- **WHEN** `COUNT(ai.transaction_id) < COUNT(*)` para o tenant/mês
- **THEN** o cron loga `[cron] tenant=<uuid> coverage=X/Y — skipped` e continua para o próximo tenant

### Requirement: Cron itera por tenants sem RLS global
O cron SHALL consultar `SELECT id FROM tenants WHERE status = 'active'` sem `SET LOCAL` (acesso direto à tabela `tenants`) e iterar por cada tenant aplicando `SET LOCAL` individualmente para cada uma das queries de cobertura e geração de digest.

#### Scenario: Múltiplos tenants ativos
- **WHEN** o cron roda e há 3 tenants ativos
- **THEN** o cron processa cada tenant em sequência, gerando digest apenas para os que estão prontos
