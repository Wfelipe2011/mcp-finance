...
export async function handleCreateWorker(req: Request): Promise<Response> {
...
export async function handleListWorkers(req: Request): Promise<Response> {
  const auth = await requireSuperAdmin(req);
  if (!auth.valid) return errorResponse("Forbidden", auth.status);
...
export async function handleUpdateWorker(req: Request, url: URL): Promise<Response> {
  const auth = await requireSuperAdmin(req);
  if (!auth.valid) return errorResponse("Forbidden", auth.status);
...
export async function handleDeleteWorker(req: Request, url: URL): Promise<Response> {
  const auth = await requireSuperAdmin(req);
  if (!auth.valid) return errorResponse("Forbidden", auth.status);
